...
def main():
    ...
    tools=load("tools.json"); guides=load("guides.json"); recs=load("recommendations.json"); accelerator=load("accelerator.json"); editorial=load("editorial_calendar.json", {"seasonal_pages": [], "traffic_channels": [], "source_action_items": []}); editorial=load("editorial_calendar.json", {"seasonal_pages": [], "traffic_channels": [], "source_action_items": []}); editorial=load("editorial_calendar.json", {"seasonal_pages": [], "traffic_channels": [], "source_action_items": []}); editorial=load("editorial_calendar.json", {"seasonal_pages": [], "traffic_channels": [], "source_action_items": []})
    ...
    for entry in editorial.get("seasonal_pages", []):
        page_actions.append({"score":26+len(entry.get("recommendation_ids", []))*2,"title":entry["title"],"path":f"seasonal/{entry['slug']}","reason":"seasonal buyer-intent page from supplied affiliate strategy"})
    gaps=[]
